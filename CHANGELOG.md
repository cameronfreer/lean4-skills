...
Parametrized debate engine — a single shared multi-perspective deliberation module, instantiated for `/lean4:draft`, `/lean4:review`, and `/lean4:learn` (Refs #151: dispositions the worthwhile confidence-gated debate ideas from PR #119 into a factored, opt-in design). Every instance is a choice of eight parameters; nothing runs without a trigger firing **and** the user opting in.

### Added

- **`references/debate-engine.md`** — the module: a panel of 2–4 named perspectives each answering one question with a committed position, an adjudication rule, and a one-line outcome note, parametrized by `panel` / `substrate` / `rounds` / `evidence` / `trigger` / `gate` / `adjudication` / `outcome`. Design rules in priority order: **opt-in** (under the default `--debate=ask` gate a debate is *proposed*, never auto-run; `yes`/`no` decide the occurrence, `always`/`never` persist for the session; a proposal that cannot be shown counts as `no` and is never silently coerced to `auto`), **bounded** (fixed rounds and evidence budgets; trigger detection must be free), **advisory** (feeds an existing decision point, never adds side effects — read-only workflows stay read-only), **host-agnostic** (inline substrate everywhere; `subagent` is reserved and degrades to inline). An instance matrix covers the workflow family: draft/review/learn implemented now; prove/refactor/golf planned; formalize composes the draft + prove instances; **`autoprove`/`autoformalize` deliberately deferred** — they cannot prompt mid-run, and consent gates are exempt from the usual `ask`→`auto` startup coercion, so wiring them awaits a startup-time opt-in design.
- **Draft instance — formalization debate** (`/lean4:draft --debate=ask|auto|off`, new Actions step 4). Triggers: materially different candidate formalizations survive drafting; faithfulness risk (vacuous / trivially true / weaker-stronger than the informal claim); repeated strict-elab failures implicating statement shape. Panel Fidelity / Idiom / Provability, one inline round, ≤2 new LSP calls; adjudication Fidelity > Idiom > Provability; viable losers become depth-check "alternative formalization" entries. `--debate` is parser-validated: new shared `debate_flag()` factory in `lib/command_args/specs/_common.py` (future prove/refactor/golf/formalize instances reuse it), wired into `specs/draft.py` with parser golden tests.
- **Review instance — finding adjudication** (`/lean4:review --debate=ask|auto|off`, batch mode only; review is not parser-covered, so the flag is model-parsed). Triggers: findings with uncertain evidence, contested severity/category, or destructive remedies. Panel Advocate / Skeptic / Maintainer over already-collected analysis output (0 new tool calls); one proposal covers all triggered findings; failed findings move to a visible "Set aside by adjudication" list. Boundaries pinned: stuck mode never runs debates or proposals; adjudication filters findings the existing analyses produced — no new finding sources (#110's scope); `--json` schema unchanged (markers deferred to #115); read-only throughout.

### Changed

- **`/lean4:learn` pedagogical self-debate rewired, behavior-preserved** — step 5 and `learn-pathways.md` now name it as the learn instance of the shared engine (panel Pace/Method/Depth; `always-inline` gate, valid because it spends zero tool calls and its only direct output is the `*Pedagogy: ...*` note, with effect scope already gated by `--adaptive` and explicit-flag precedence). learn-pathways.md remains the normative behavior spec; the engine reference adds only the parameter mapping.
- draft.md line budget 170→190, review.md 340→360; `SKILL.md` reference index gains the debate-engine pointer. Contract test Check 36 pins the parameter table, the opt-in gate invariants, the draft/review wiring (including the parser spec), the learn mapping, and the autonomous-command deferral.
