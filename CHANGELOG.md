...
- **`--prior-run <id>` on `prove` / `autoprove`** (requires `--persist`; resolved within the selected store; never "latest"). A **new** invocation receives the prior run's historical evidence, reconciles source drift before taking fresh custody, and applies the rerun guard before repeating blocked work. Not resume, not journal repair, no lock recovery, no automatic drift acceptance.
- **`lean4-skills-run-persist reuse` / `custody`** (`lib/scripts/run_reuse.py`, read-only): selects the last dispatch (or the manifest's, cited `<run_id>#manifest`), the **latest recorded handoff with finality unknown** (the cache is disposable and irrelevant), and the baseline with its origin; preserves the selected handoff's blocker/stop fields as evidence (Replan blockers supplement, never replace; with no usable handoff the handoff-based guard is reported as not evaluable); checks target/mode/owned-file compatibility; produces a **content-bound** drift report whose approval token digests existence, resolved path and content hash per file, which `custody` re-derives over all intended owned files before handing back the fresh baseline (`custody_mismatch` if anything changed again). Baselines are merged **per file in journal order** (the newest dispatch/handoff entry for a path wins; older records fill paths a partial worker handoff or a later redispatch does not cover) and an intended file with no prior entry is an explicit `uncovered` outcome needing the same approval — never an implicit match. The report is bound to its invocation (project root, target, scope, mode, ownership set — all compared against the actual startup dispatch): `custody` and `start` refuse another (`reuse_report_mismatch`), `start` re-checks compatibility against the actual dispatch, a malformed report is `bad_reuse_report`, the observation digest covers the manifest, and the historical material (review recommendations, note texts, snippet sources, every Replan's failed approaches, the prior handoffs' best candidates) is rebuilt from validated records and reaches the new run. `start` re-derives custody at the final boundary (`--approve <token>` for a guided non-match; autonomous mode refuses drift — `drift_unreconciled`) and requires the first dispatch's baseline to be the fresh one custody returned, compared by full recorded identity per path — realpath, existence, hash, size (`baseline_mismatch`). Target compatibility uses path semantics (same file for `file` scope, whole-component containment for `project`/`changed`). Chained reuse stays flat: a prior helper-generated source-note (`run-persist-source-note/v1`, validated in shape; other JSON-shaped notes stay text) is inherited as structured data with original citations (`historical.inherited` links carrying plan, next steps and the ancestor's carried blockers as historical only; items marked `inherited_via`), never nested as serialized text. Unusable prior runs (unknown, foreign, incomplete, damaged, no baseline, locked, incompatible) are startup validation errors with nothing created; a present lock refuses and is never broken, and the absence of one proves nothing — the preview names the observed journal prefix and `start` revalidates it (`prior_run_changed`).
- **`run-persist start --prior-run --reuse-report`** evaluates the shipped rerun guard against the selected handoff before creating anything (`rerun_forbidden` echoes `new_evidence_required_for_rerun`; `blocker_cleared` if the first dispatch drops the prior signature; an operational prior stop needs a specific `--evidence-justification` — a new run id, approved drift or a fresh baseline is not evidence), creates the run naming `prior_run`, and records **one local `source-note`** listing the selected material with its original citations; Replan cites stay same-run and cite that note. Historical items are presented as dead ends under prior assumptions (not prohibitions), completed reviews with disposition unknown, snippets unverified.
- **Docs and checks.** cycle-engine.md `## Prior-Run Reuse` (canonical), flag rows and startup steps in both commands (autoprove stops before any edit and before custody on drift), handoff-contract and run-store notes; Check 45 pins the policy. Acceptance `tests/integration/test_prior_run_reuse.py` (Linux/macOS; portable selection on Windows): positive reuse end to end, source-note write outcomes under failed bookkeeping (committed/refused/indeterminate), partial worker baseline, uncovered file, invocation binding, historical content, malformed report, manifest digest, cache irrelevance, drift refusal and content-bound approval (a file changed again after approval aborts), unchanged-blocker refusal without drift, blocker-cleared refusal, operational-stop justification, intermediate/missing/no-baseline handoffs, cross-run citation refusal, changed prior run, lock, incompatibility, damaged journal.
...
- **`prove` / `autoprove --persist`** (default off; existing invocations unchanged) with `--run-store DIR` (requires `--persist`; precedence `--run-store` → `$LEAN4_RUN_STORE` → `<project-root>/.lean4-skills`). One run per invocation; the parent/controller is the sole journal writer. Startup order: inputs → capability checks (platform support is a startup check, so an unsupported host is a startup validation error and requested persistence never silently disappears) → a valid first dispatch → `start` before any proof edit. Every review the command runs, skips, or fails becomes a `review` event; every cycle boundary writes a `replan` event before `tick`; typed notes at the point they occur; the final handoff via `finish`. Stored items are cited only as `<run_id>#<seq>`. Canonical prose: cycle-engine.md § Run Persistence; both command docs link it and add the flags (first `command_args` spec entries for these flags, with parser tests).
- **`lean4-skills-run-persist` / `lib/scripts/run_persistence.py`** — the protocol helper that drives the store and encodes the failure policy as code: `committed` → continue; `journal_only` → continue with a warning, never re-append; `busy` → one bounded retry, never break the lock; any other refusal or an `indeterminate` write → stop proof work and emit an operational-error handoff (an event visible to `load` is not evidence of commit — no retry, no inferred success, no automatic reconciliation); a failed final `finish` returns the fallback handoff to be printed in full and never claimed stored. Once stopped, later calls return `stop` without touching the store.
- **`run-store-manifest/v2` + `run-store-event/v2`** — an explicit envelope version, not a quiet extension: manifest v2 requires `event_schema` (missing ⇒ invalid, never implicitly v1); the event kind enum adds `review` (`review-record/v1`: `status: completed|skipped|failed`, a completed batch review carries the shipped `lean4-review-output/v2` verbatim, a completed stuck review carries the triage and the parent's mapped handoff, skipped/failed carry only a reason) and `replan` (`replan-summary/v1`; `cites` must resolve to earlier events of the same run). Runs are single-version; writers select the event schema from the manifest, never rewrite it or upgrade a journal; a v2 kind in a v1 run is refused before writing (the run stays valid, an empty journal included); readers accept both and report mixed-version journals as corrupt. `run-store-handoff/v1` and `run-contract/v1` unchanged. Second fixture run (v2) beside the v1 one.
- **Review round 1 (helper state and truthful failure handoffs).** The invocation state is control state: `start` creates it exclusively and binds the storage root and run id; every mutation is recorded in-flight before the store is invoked and resolved afterwards, so a state-write failure or a death in between leaves an unresolved record that stops the next call — bookkeeping failure never suppresses the store's outcome and never lets the run continue; `finish` and a policy stop are terminal. The operational-error handoff is built from the current context (latest persisted dispatch, accumulated `files_changed`, baseline and evidence from persisted handoffs and notes) and validated; an invalid `finish` submission is refused before anything is sent and the fallback is the validated operational record. Store acknowledgments are validated (schema, exit status vs outcome, run id, seq) — anything else is uncertain and never yields a citation. `--run-store` with `--persist=false` is now a startup error (the companion rule is judged on the resolved value); `validate --kind event` no longer raises on a non-string schema; review records enforce cross-field rules (a completed report has no error; a stuck review's mapped handoff wraps that triage of that target and is blocker-driven). Command docs de-duplicated and the state setup documented with an executable sequence. Round 2: a committed write whose resolution cannot be recorded stops the current call (the outcome and citation are kept under `committed`) rather than authorizing one more step; parent knowledge is distinguished from committed history — a validated dispatch/handoff/note whose own write fails or is unresolved still informs the operational-error handoff (changed files, baseline, evidence, artifacts) and is reported as `unpersisted` without a citation. Round 3: that knowledge also informs the fallback when the in-flight record itself could not be saved (the store was never called; the result says the pending knowledge will not survive another invocation); a committed submission whose resolution save failed is folded into the response exactly once; and `unpersisted` distinguishes known non-storage (`not-stored`, a pre-write refusal) from an unconfirmed write (`unconfirmed`, no citation available from this state). The same distinction applies to a `finish` that could not be stored (`persistence: not-stored | unconfirmed`). Round 4: `review-record/v1` accepts `source: none` only for a skipped review, so `--review-source=none` is persisted as configured; every failed-`finish` result goes through one constructor and the cycle-engine / autoprove instructions report stored / not-stored / unconfirmed accordingly; the terminal guarantee is scoped honestly — when no control-state write succeeded the result carries `terminal_enforced: false` and warns that a later helper process cannot be guaranteed to remember the stop; the integration suite identifies the journal by device/inode instead of `/proc` and runs on the macOS and Windows jobs; autoprove's startup requirements name the persistence gate, and the state-path example uses `mktemp -d` rather than `$TMPDIR`.
- **Tests.** `tests/integration/test_run_persistence.py`: a two-cycle run through the real helper, store and cycle tracker, then loaded and checked (dispatches, notes, a batch and a stuck review, two replans, a final handoff whose citations resolve); persistence off ⇒ no storage activity; unsupported platform ⇒ startup error before any edit; refusal before replan ⇒ stop, no next cycle; `journal_only` ⇒ one event, no duplicate; visible-but-unsynced ⇒ stop, not continue; `busy` ⇒ one retry; final-handoff failure ⇒ visible fallback, never claimed stored; no store result ⇒ stop. Store suite 70 → 80 (v2 shapes, review combinations, cites, v2-kind-in-v1 refusal on an empty journal, mixed-version corruption, both fixtures). Check 44 pins the prose to the protocol; Check 43 extended for the versions.

## v4.9.0 (September 2026)

Run-store/v1 (#82A; Refs #82): the storage primitive for persisted proving state. Storage only — command wiring, Replan updates and restart recovery stay open under #82.

### Added

- **`lean4-skills-run-store` / `lib/scripts/run_store.py`.** One directory per run under `<storage_root>/runs/<run-id>` (`storage_root = $LEAN4_RUN_STORE` or `<project-root>/.lean4-skills`): an immutable `manifest.json` (`run-store-manifest/v1`, carrying the first `run-contract/v1` dispatch verbatim, a nullable tracker session id, and a nullable `prior_run`), an append-only `events.jsonl` journal (`run-store-event/v1`: dense `seq`, kinds `dispatch`/`handoff`/`note`, payloads are unchanged `run-contract/v1` records), and a disposable `handoff.json` cache the journal always overrides. Journal integrity: only an unterminated final fragment is recoverable; terminated garbage, schema-invalid events, `seq` gaps or duplicates are corruption; appending to a damaged journal is refused and recovery is a new run naming `prior_run`. Reads are validated observed prefixes (no reader lock; a concurrent tail fragment is not proof of a crash); the effective handoff is the last handoff event in the prefix and the cache is current only when its `seq` and payload match it. Durability is stated per operation (write loop, `fsync`, temp → `rename` via directory fd → directory `fsync`; macOS `F_FULLFSYNC` attempted) with four honest write outcomes — `committed`, `journal_only`, `nothing_written`, `indeterminate` — and no exactly-once claim. One `O_EXCL` lock per mutation; any existing lock blocks; no automatic breaking. Mutations are implemented for Linux/macOS (per-component `O_NOFOLLOW` opens relative to the parent descriptor); other platforms are refused before any write. `runs/.gitignore` (`*`) is written only if absent, and creation refuses when paths under `runs/` are already in the Git index. Loaded content is historical evidence, never certification: no trust promotion, no custody check, no rerun-guard enforcement.
- **`lib/scripts/run_contract_validate.py`.** The run-contract/v1 validator moved from the test suite to production and hardened for arbitrary JSON (a non-object root, lists where strings/enums are expected, a numeric `stop_detail`, an unhashable worker name all yield field-naming errors, never exceptions); `tests/test_run_contract.py` imports it. Structural validation is separate from the rerun predicate, which the store never calls.
- **Review round 1 (storage edge cases).** Root containment now anchors on the *parent* of `storage_root` and opens the root entry itself with `O_NOFOLLOW`, so a symlinked `project/.lean4-skills` is refused and manifests record the resolved root; fresh-store initialization synchronizes `storage_root` and its parent after creating the new entries; `runs/.gitignore` is published by unique temp + `link` so a failed initialization leaves no empty file to be mistaken for user policy; the Git index check runs against the repository that contains the storage location, not only the project; a failed write of the lock's own contents releases the lock (`lock_init_failed`) instead of leaving the run permanently busy; expected filesystem refusals (symlinked journal, …) are structured `containment` outcomes and an unexpected `OSError` is reported as `indeterminate`, never as `nothing_written`; cache temps are uniquely named and removed on a failed rename, so one failure never blocks later replacements; a manifest whose `run_id` names another run is `integrity_failure`; `load`/`validate` have a portable read-only path backend (Windows CI job loads the prebuilt fixture and checks that a mutation is refused without creating anything; a macOS job runs the full storage suite); `F_FULLFSYNC` is applied to file descriptors only; `--now` must be exactly `YYYY-MM-DDTHH:MM:SSZ` and ids are zero-padded from components. Process-kill tests (SIGKILL at a chosen `fsync`) now sit beside the injected-exception tests. The Windows job then found the CLI crashing on a cp1252 console when a result contains `⊢`: results are now written as UTF-8 bytes and payloads read as UTF-8 bytes regardless of the console (reproduced on Linux with `PYTHONIOENCODING=cp1252`). Round 2: the store-initialization barriers (`fsync(storage_root)`, `fsync(parent)`) are repeated on every `create` — an existing directory does not prove an earlier publication was synchronized — and the parent of the storage root must already exist rather than being created as an unsynchronized ancestor chain; the macOS/Windows jobs run the suite directly (a `| tail` pipeline without `pipefail` had masked one failing resolved-path assertion on macOS). Round 3: every mutation re-establishes the run-directory and `runs`-directory publication barriers under the lock before accepting a journal write (`publish_unsynced` otherwise) — a loadable run left by a `create` that failed at its last barriers no longer acknowledges an append while its directory entries may be undurable — and `create` holds its lock through its final barrier; persisted baseline paths are validated by a host-independent serialized grammar instead of the reader's `os.path` (Python 3.13's Windows `isabs` rejected the POSIX-written fixture; the platform jobs now run on 3.10 and 3.13); an unusable cache (directory, FIFO, read error) is always just `handoff_cache_stale`, with non-blocking opens and a regular-file check; unpaired surrogates are refused before any filesystem change and escaped on output. Round 4: the complete manifest (including a storage root whose directory name carries a non-UTF-8 byte) is preflighted before the run directory is reserved.
- **Tests and checks.** `tests/test_run_store.py` (stdlib): round-trip, validator field sweeps, tail-vs-corruption, append-after-damage refusal, stale/mismatched/ahead caches, short-write retry, partial-write and `fsync`/`F_FULLFSYNC`/`rename`/directory-sync failures at each step of append, set-handoff and create, competing lock acquisition via the CLI, lock release on handled failure, symlinked `runs`/run/journal refusal, unsupported-platform refusal before writing, `.gitignore` preservation, indexed-path refusal, exit codes. Wired into the python-tests job and the wrapper-smoke table. `references/run-store.md` is the contract; Check 43 pins its load-bearing sentences and the wiring (supporting the tests, not replacing them).
...
- **Executable evidence.** `tests/fixtures/reference_snippets/diagnostic_snippets.lean` elaborates every repair and pins every failure as a `#guard_msgs` control against the pinned Lean 4.33.1 toolchain; the `omit` failure is a parse error `#guard_msgs` cannot capture, so `diagnostic_omit_negative.lean` must fail and `run_core_snippets.sh` asserts the message and the docstring-line location. The `lean-integration` workflow runs the script (which also covers the v4.8.6 instance fixture). Check 42 pins discoverability only: the rows, the § 20/§ 21 homes, the links from the older copies, the "can eliminate" phrasing, and the fixtures' presence in CI. Review: the runner matches file, line, severity, and message in one grep (an unrelated line-7 diagnostic beside the omit error elsewhere no longer passes) and prints the full diagnostic on failure without a `head` pipe under `pipefail`; the generic "unknown identifier" row and § 5 cross-link the `rintro … rfl` pitfall for a vanished local.
