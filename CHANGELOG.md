...
- **Docs and checks.** cycle-engine.md `## Prior-Run Reuse` (canonical), flag rows and startup steps in both commands (autoprove stops before any edit and before custody on drift), handoff-contract and run-store notes; Check 45 pins the policy. Acceptance `tests/integration/test_prior_run_reuse.py` (Linux/macOS; portable selection on Windows): positive reuse end to end, cache irrelevance, drift refusal and content-bound approval (a file changed again after approval aborts), unchanged-blocker refusal without drift, blocker-cleared refusal, operational-stop justification, intermediate/missing/no-baseline handoffs, cross-run citation refusal, changed prior run, lock, incompatibility, damaged journal.
