...
- **Review round 1 (helper state and truthful failure handoffs).** The invocation state is control state: `start` creates it exclusively and binds the storage root and run id; every mutation is recorded in-flight before the store is invoked and resolved afterwards, so a state-write failure or a death in between leaves an unresolved record that stops the next call — bookkeeping failure never suppresses the store's outcome and never lets the run continue; `finish` and a policy stop are terminal. The operational-error handoff is built from the current context (latest persisted dispatch, accumulated `files_changed`, baseline and evidence from persisted handoffs and notes) and validated; an invalid `finish` submission is refused before anything is sent and the fallback is the validated operational record. Store acknowledgments are validated (schema, exit status vs outcome, run id, seq) — anything else is uncertain and never yields a citation. `--run-store` with `--persist=false` is now a startup error (the companion rule is judged on the resolved value); `validate --kind event` no longer raises on a non-string schema; review records enforce cross-field rules (a completed report has no error; a stuck review's mapped handoff wraps that triage of that target and is blocker-driven). Command docs de-duplicated and the state setup documented with an executable sequence.
