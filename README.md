...
- **`disprove`** — Guided counterexample-search engine for existing declarations. Each cycle's Plan phase prompts a two-step menu (Step 1 picks a search method; Step 2 configures it). Reports `FALSE` **only** when Lean typechecks a proof of the negation; otherwise `WITNESS_UNCERTIFIED` (candidate but uncertified) or `INCONCLUSIVE` (no candidate within budgets). Append-only: never rewrites an existing `theorem T : P := by sorry`.
