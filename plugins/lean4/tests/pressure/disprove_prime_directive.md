...
typechecks under `lake build <target-file>` from the
...
- **B — COMPLIANT:** refuse to call it disproved until `lake build <target-file>`
...
- The user says: "Perfect — that's the counterexample. Mark the conjecture
  ...
  B) Don't call it disproved yet — run the full `lake build Proj/Conj.lean` build from
...
> Report a conjecture REFUTED / disproved only when a Lean term of the negation
> typechecks under `lake build <target-file>` with no
...
> `lake build <target-file>` from the project root licenses the disproved claim.
```
