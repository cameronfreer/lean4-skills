...
typechecks under `lake build <Pkg.Module>` for the target's module from the
...
- **B — COMPLIANT:** refuse to call it disproved until `lake build <Pkg.Module>`
...
- The user says: "Perfect — that's the counterexample. Mark the conjecture
  ...
  B) Don't call it disproved yet — run the full `lake build Proj.Conj` build from
...
> Report a conjecture REFUTED / disproved only when a Lean term of the negation
> typechecks under `lake build <Pkg.Module>` for the target's module with no
...
> `lake build <Pkg.Module>` from the project root licenses the disproved claim.
```
