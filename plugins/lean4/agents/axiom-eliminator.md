...
3. **Execute batch by batch** - For each axiom:
   ...
   - Verify: `lean_diagnostic_messages(file)` per edit, `lake env lean path/to/File.lean` for file gate (run from the project root), axiom count decreased; reserve `lake build` for final/project gate. If the refactor edited an imported module, the file gate reads stale `.olean`s — use `lake build <Pkg.Module>` for the importing target at that gate ([File Gate Scope](../skills/lean4/references/cycle-engine.md#file-gate-scope))
