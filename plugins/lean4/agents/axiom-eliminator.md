...
3. **Execute batch by batch** - For each axiom:
   ...
   - Verify: `lean_diagnostic_messages(file)` per edit, `lake env lean path/to/File.lean` for file gate (run from the project root), axiom count decreased. Two distinct builds: `lake build <path/to/File.lean>` (targeted; the dependency-consistent gate after this refactor edited an imported module, since the file gate reads stale `.olean`s — [File Gate Scope](../skills/lean4/references/cycle-engine.md#file-gate-scope)) and plain `lake build` (project-wide; reserve for the final/project gate)
