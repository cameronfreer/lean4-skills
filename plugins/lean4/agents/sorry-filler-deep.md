...
- **File-baseline drift check (issue #102) — fail closed.** A valid, nonempty `### File baseline` record is required before any mutation; if it is missing, malformed, or the checker is unavailable, apply no mutation and report why. Run `lean4-skills-file-baseline check --baseline -` (record over stdin) immediately before every mutating tool operation, checking every intended target first for multi-file operations — **only exit 0 authorizes the next mutation**. On any nonzero exit (drift or operational error): apply no mutation, report the structured stale-baseline result (affected paths + classification + recommend `rerun`, `serialize`, or `isolation: "worktree"`), and stop — never re-record and retry. After each successful mutation, advance only the entries you intentionally changed (`advance --baseline - <changed paths>`); if advance fails, stop before any further mutation. Full custody rule: cycle-engine.md § File baselines and drift.
