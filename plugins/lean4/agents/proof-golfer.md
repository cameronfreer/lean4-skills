...
2. **Verify safety** before inlining any binding:
   ...
   > **MCP canary:** Before step 3, test `lean_diagnostic_messages(file)`. If unavailable (tool-not-found, missing from context, or inaccessible), emit "⚠ Lean MCP tools unavailable — golfing limited to syntactic patterns", skip steps 3-4 (require `lean_multi_attempt`), and proceed with step 5 using `lake build` for validation. Baseline-comparison gate cannot run without per-edit diagnostics — scope is reduced accordingly.
