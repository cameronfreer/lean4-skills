...
`lake build <target-file>` (the dependency-consistent
...
- **No `REFUTED` without compile gate + axiom gate.** `lean_multi_attempt` and
  `lake env lean` are pre-screens; `REFUTED` requires `lake build <target-file>`
  (from the project root, on the resolved source path; rebuilds stale imports, so
  the check is dependency-consistent) to succeed (no `sorry`/`admit`) **and** the `¬ TARGET` declaration's axioms
