...
`lake build <Pkg.Module>` for the target's module (the dependency-consistent
build; `lake env lean` is a pre-screen, never the license — it reads built
...
- **No `REFUTED` without compile gate + axiom gate.** `lean_multi_attempt` and
  `lake env lean` are pre-screens; `REFUTED` requires `lake build <Pkg.Module>` for
  the target's module (from the project root; rebuilds stale imports, so the check
  is dependency-consistent) to succeed (no `sorry`/`admit`) **and** the `¬ TARGET` declaration's axioms
