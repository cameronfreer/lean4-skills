...
- **Docstring policy — Rule C (new-file / new-decl generation).** Draft may emit a module docstring on a file it newly creates and a declaration docstring on a declaration it newly creates. It must not rewrite docstrings on declarations that already exist in a target file — those stay under Rule A. New-file module docstrings follow the canonical header template in [mathlib-style.md § 1](../skills/lean4/references/mathlib-style.md#1-file-header-copyright-module-imports-critical) (Rule C fills that template's docstring slot; it does not redefine the template).
