...
2. **Otherwise consult the shared project-context helper.** Run `lean4-skills-project-context --from <dir>` where `<dir>` is the nearest existing parent directory of the `--out` target (the target itself does not exist yet). Require `schema` equal to `project-context/v1` in the JSON output, `intent.contributing_upstream` a string in `yes | no | unknown`, and `intent.source` a string in its documented domain; any missing, non-string, or out-of-domain value is malformed helper output.
