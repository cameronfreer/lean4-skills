...
1. **Explicit flag wins.** `--mathlib-template` → emit it; `--no-mathlib-template` → do not. Only a flag resolving to true participates in precedence; explicit false (e.g. `--mathlib-template=false`) is equivalent to omission.
2. **Otherwise consult the shared project-context helper.** Run `lean4-skills-project-context --from <dir>` where `<dir>` is the nearest existing parent directory of the `--out` target (the target may not exist yet; passing a nonexistent target directly would exit 4). Require `schema` equal to `project-context/v1` in the JSON output, `intent.contributing_upstream` a string in `yes | no | unknown`, and `intent.source` a string in its documented domain; any missing, non-string, or out-of-domain value is malformed helper output.
...
4. **Helper failure degrades to `unknown`.** Missing wrapper, nonzero exit, malformed JSON, wrong `schema`, or a malformed intent record per step 2 → treat as `unknown` with source `helper-failure` (advisory included). The gate never blocks a write.
...
When the gate fires, the emitted file shape is: copyright block, then `module`, then the `public import` block, then the plain `import` block, then the `/-!` module docstring, then a `public section` opening the exported scope before any declarations. Declarations in a `module` are private by default, and `public import` alone re-exports imports without making this file's declarations public — a file missing the `public section` (or per-declaration `public`) elaborates yet exports nothing. Use `@[expose] public section` only when downstream definitional unfolding is intentionally part of the API; plain `public section` exports signatures while keeping implementations opaque. Preserve the selected import set — assign `public import` to public-API dependencies (statements and signatures are the central case; exported instances, notation, syntax, and metaprogramming can also require it, and Lean's own public/private checking is authoritative over this heuristic) and plain `import` to implementation/proof-only dependencies, alphabetized within each block with a blank line between the two nonempty blocks (omit a block rather than emit an empty group); the gate changes header shape, never import selection. For the copyright block, use the current year and `git config user.name` as best-effort defaults — user-supplied attribution always wins, lookup failure falls back to the `Author Name` placeholder and never blocks the write, and the chosen value is reported in the output summary.
...
- **Docstring policy — Rule C (new-file / new-decl generation).** Draft may emit a module docstring on a file it newly creates and a declaration docstring on a declaration it newly creates. It must not rewrite docstrings on declarations that already exist in a target file — those stay under Rule A. When the [Mathlib Template Gate](#mathlib-template-gate) selects the mathlib header, Rule C fills that template's module-docstring slot in [mathlib-style.md § 1](../skills/lean4/references/mathlib-style.md#1-file-header-copyright-module-imports-critical) shape; Rule C neither selects nor redefines the template.
