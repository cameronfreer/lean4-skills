...
- **Docstring policy — Rule C (new-file / new-decl generation).** Formalize may emit a module docstring on a file it newly creates and a declaration docstring on a declaration it newly creates; it must not rewrite docstrings on pre-existing declarations (those stay under Rule A). New-file module docstrings follow the canonical header template in [mathlib-style.md § 1](../skills/lean4/references/mathlib-style.md#1-file-header-copyright-module-imports-critical) (Rule C fills the docstring slot, it does not redefine the template).
