...
- **Docstring policy — Rule C (new-file / new-decl generation).** Formalize may emit a module docstring on a file it newly creates and a declaration docstring on a declaration it newly creates; it must not rewrite docstrings on pre-existing declarations (those stay under Rule A). When the [Mathlib Template Gate](#mathlib-template-gate) selects the mathlib header, Rule C fills that template's module-docstring slot in [mathlib-style.md § 1](../skills/lean4/references/mathlib-style.md#1-file-header-copyright-module-imports-critical) shape; Rule C neither selects nor redefines the template.
