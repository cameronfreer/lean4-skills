...
- After a successful mutation, the writer advances **only the entries it intentionally changed** (`advance --baseline - <changed paths>`); untouched-file entries are carried forward unchanged, so external drift on them is never blessed. **The JSON emitted by `advance` replaces the previous current baseline** and must be used for the next `check` or `advance` — checking against a superseded baseline mistakes your own accepted edit for external drift.
