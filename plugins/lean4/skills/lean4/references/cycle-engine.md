...
- **Fail closed.** Editing agents require a valid, nonempty baseline before any mutation: a missing `### File baseline` field, malformed or empty record, unavailable checker, or **any nonzero check exit** prevents mutation — **only exit 0 authorizes the next mutating operation**. (The primitive enforces its side: empty records and malformed entries are rejected as input errors, never treated as a match.)
