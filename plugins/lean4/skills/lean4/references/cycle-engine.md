...
**Drift and custody.** The preview's drift report is content-bound: per intended owned file it carries existence, resolved path, and the current content hash, plus the prior baseline's check statuses; its `approval_token` is the digest of that exact report. `custody` re-derives the report over **all** intended owned files and requires the fresh token to equal the approved one — a file that changed again after approval (same list, different bytes) aborts with `custody_mismatch`; only then is the fresh baseline handed back for the first dispatch. Guided `prove` shows the particular changes and asks the user to approve **that report** (the token); autonomous `autoprove` **stops before any edit and before custody** on drift, emitting an operational-error handoff naming what needs reconciliation — it never prompts and never accepts drift automatically. Missing files are drift (`deleted`).

**The rerun guard is preserved.** The first dispatch must carry the prior blocker (`prior_blocker` equal to the selected handoff's signature; `blocker_cleared` otherwise). `start` evaluates the shipped predicate against the selected handoff: same task, same blocker, empty delta → `rerun_forbidden` with the prior `new_evidence_required_for_rerun` echoed, nothing created. **A new run id, approved drift, or a freshly recorded baseline is not evidence.** For an operational prior stop, `--evidence-justification` must state specifically how the delta addresses the recorded `stop_detail` (a generic phrase is refused).
