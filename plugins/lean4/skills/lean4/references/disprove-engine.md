...
the negation that typechecks under `lake build <target-file>` for the target's
...
| Compile gate (`lake build <target-file>`) + axiom-whitelist gate | **model-mediated (tool calls)** | Checkpoint runs `lake build <target-file>` (dependency-consistent; `lake env lean` is only a pre-screen) + `#print axioms` / `lean_verify` |
...
| 3. Checkpoint | If Work produced a **pre-screen-passing candidate**: append `T_counterexample`, run `lake build <target-file>` + axiom check (⊆ whitelist) → `certified` (`REFUTED`) only if both pass, else revert hunk → `near-miss`/`WITNESS_UNCERTIFIED`; stage + commit per `--commit`. If no candidate: no artifact. |
...
`<target-file>` Phase 3 appends `T_counterexample` to and builds (`lake build <target-file>`)
...
| `near-miss` | A candidate passed pre-screen but the Phase 3 gate rejected it — `lake build <target-file>` failed, OR a non-whitelisted axiom appeared / axiom inspection was inconclusive (→ `WITNESS_UNCERTIFIED`). The error signature is captured. |
...
3. **Compile gate.** Run `lake build <target-file>` from the project root, passing
   the resolved target's source path exactly as Phase 1 resolved it — Lake accepts a
   source path as a build target and resolves the module through the workspace
   config (never convert the path to a module name by hand; that breaks on custom
   `srcDir` layouts). This is unconditional: it rebuilds any stale import, so
   certification cannot rest on a source/`.olean` mismatch that predates the run.
   `lake env lean <target-file>` (also from the project root) may be used as a
   faster pre-screen before it, but it checks only the built `.olean`s and never
   licenses `REFUTED` ([File Gate Scope](cycle-engine.md#file-gate-scope)).
...
5. License the outcome:
   - **`certified` (→ `REFUTED`)** only if the `¬ TARGET`-typed declaration
     ...
     **before** the commit, then from the project root re-run
     `lake build <target-file>` on the wrapper-free file, so the committed state
...
- **No claim of `REFUTED` without compile gate + axiom gate.** Pre-screen via
  ...
  `REFUTED` requires **both** `lake build <target-file>`
