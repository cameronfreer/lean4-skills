...
4. `lean_diagnostic_messages(file)` per-edit, `lake env lean <path/to/File.lean>` for file gate (from project root); once a refactor has touched an imported module, gate with `lake build <Pkg.Module>` instead — the file gate reads stale `.olean`s ([File Gate Scope](cycle-engine.md#file-gate-scope))
...
lake build Pkg.Module    # Instead, once an extraction moved code into an imported module (see cycle-engine.md#file-gate-scope)
```
